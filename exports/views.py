...
def google_calendar_callback(request):
    ...
    try:
        ...
        # TODO: 실제로는 이 토큰들을 데이터베이스에 저장해야 함
        # 지금은 임시로 세션에 저장
        request.session["google_access_token"] = auth_data["access_token"]
        request.session["google_refresh_token"] = auth_data["refresh_token"]
        request.session["google_email"] = auth_data["google_email"]
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def google_calendar_sync(request, plan_id):
    """여행 계획을 구글 캘린더에 동기화"""
    # 여행 계획 존재 확인
    travel_plan = get_object_or_404(TravelPlan, id=plan_id, user_id=request.user.id)
    ...
    if not access_token:
        return Response({
            "error": "구글 캘린더 인증이 필요합니다. 먼저 /api/exports/auth/google/ 로 인증해주세요."
        }, status=status.HTTP_400_BAD_REQUEST)
