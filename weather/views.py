...
# 지역별 날씨 API
class WeatherByRegionAPI(APIView):
    ...
    def get(self, request, region_id):
        try:
            ...
            first_subregion = region.subregions.first()

            if not first_subregion or not first_subregion.location:
                return Response({
                    "error": f"'{region.get_name('ko')}' 지역의 위치 정보를 찾을 수 없습니다."
                }, status=status.HTTP_400_BAD_REQUEST)

            lat = first_subregion.latitude
            lon = first_subregion.longitude
    ...
    def _transform_to_ui_format(self, raw_data, region_id, region_name):
        ...
        return {
            ...
            "tomorrow_weather": {
                "min_temperature": current.get("min_temperature", 0),
                "max_temperature": current.get("max_temperature", 0),
                "weather_condition": self._get_weather_condition(current),
            },
            "detail_info": {
                "feels_like": current.get("feels_like_temperature", current.get("temperature", 0)),
                "humidity": current.get("humidity", 0),
                "wind_speed": current.get("wind_speed", 0),
                "uv_index": current.get("uv_index", 0),
                "sunrise_time": current.get("sunrise_time", "06:00"),
                "sunset_time": current.get("sunset_time", "18:00"),
            },
            "air_quality": {
                "pm25_value": current.get("pm25", 15),
                "pm25_grade": self._get_pm_grade(current.get("pm25", 15)),
                "pm10_value": current.get("pm10", 30),
                "pm10_grade": self._get_pm_grade(current.get("pm10", 30), pm_type="pm10"),
            },
            "travel_tip": {
                "tip_message": self._get_travel_tip(current)
            }
        }
    ...
    # 여행 팁 생성
        ...
        pm25 = forecast.get("pm25", 15)
        pm10 = forecast.get("pm10", 30)
...
# 지역구별 날씨 API
class WeatherBySubRegionAPI(APIView):

    permission_classes = [AllowAny]

    def get(self, request, region_id, subregion_id):
        try:
            ...
            if not subregion.location:
                return Response({
                    "error": f"'{region_name} {subregion_name}'의 위치 정보를 찾을 수 없습니다."
                }, status=status.HTTP_400_BAD_REQUEST)

            lat = subregion.latitude
            lon = subregion.longitude
    ...
    def _transform_to_ui_format(self, raw_data, region_id, region_name, subregion_id, subregion_name):
        ...
        return {
            ...
            "location_type": "subregion",
            "full_location_name": f"{region_name} {subregion_name}",
            "current_weather": {
                "temperature": current.get("temperature", 0),
                "weather_condition": self._get_weather_condition(current),
                "humidity": current.get("humidity", 0),
            ...
            "travel_tip": {
                "tip_message": self._get_travel_tip(current, f"{region_name} {subregion_name}")
            }
        }
