...
class WeatherByRegionAPI(APIView, WeatherBaseView):
    ...
    def get(self, request, region_id):
        try:
            ...
            # DB에서 날씨 데이터 찾기
            weather = Weather.objects.filter(region_id=region_id).first()
            ...
            if weather:
                weather_data = [{
                    ...
                    "sky_condition": weather.sky_code,
                    ...
                    "pm25": weather.pm25,
                    "pm10": weather.pm10,
                    "sunrise_time": weather.sunrise_time.strftime('%H:%M') if weather.sunrise_time else "06:00",
                    "sunset_time": weather.sunset_time.strftime('%H:%M') if weather.sunset_time else "18:00",
                    "min_temperature": weather.min_temperature,
                    "max_temperature": weather.max_temperature,
                    "forecast_time": weather.forecast_time.strftime('%Y-%m-%d %H:%M')
                }]
            else:
                first_subregion = region.subregions.first()

                if not first_subregion or not first_subregion.latitude or not first_subregion.longitude:
                    return Response({
                        "error": f"'{region_name}' 지역의 위치 정보를 찾을 수 없습니다."
...
class WeatherBySubRegionAPI(APIView, WeatherBaseView):
    permission_classes = [AllowAny]
    ...
    def get(self, request, region_id, subregion_id):
        try:
            ...
            if not subregion.latitude or not subregion.longitude:
                return Response({
